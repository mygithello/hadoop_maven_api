...
public class WordCountHA {
	 ...
	 */
	static {
		String[] hosts = SystemConfig.getPropertyArray("com.demo.hadoop.host", ",");
	...
	private static void run(String name) throws Exception {
		...
		// 设置统计文件在分布式文件系统中的路径
		String tmpLocalIn = SystemConfig.getProperty("com.demo.hadoop.input.path");
		String inPath = String.format(tmpLocalIn, name);
		// 设置输出结果在分布式文件系统中的路径
		String tmpLocalOut = SystemConfig.getProperty("com.demo.hadoop.output.path");
